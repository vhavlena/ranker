    ...
    map<int, int> reachCons, map<DFAState, int> maxReach, BackRel& dirRel, BackRel& oddRel, bool eta4, set<int> finals)
{
  ...
  std::cerr << "Fin size for this condition: " << fin.size() << std::endl;
  ...
  {
    std::cerr << "Returning early from succSetSchTightReduced... " << std::endl;
    std::cerr << "rankbound S: " << this->rankBound[state.S].bound << ", rankbound S': " << this->rankBound[sprime].bound << ", maxrank: " << state.f.getMaxRank() << std::endl;
    return ret;
  }
  std::cerr << "rankbound S: " << this->rankBound[state.S].bound << ", rankbound S': " << this->rankBound[sprime].bound << ", maxrank: " << state.f.getMaxRank() << std::endl;
  ...
  std::cerr << "Max ranks vector size: " << maxRanks.size() << std::endl;
  for (auto& r : maxRanks)
  {
    set<int> oprime_tmp;
    ...
    ret.push_back({sprime, oprime_tmp, r, iprime, true});
    std::cerr << "Insert into ret" << std::endl;  
  }
  ...
  while(stack.size() > 0)
  {
    ...
    for(int sym : alph)
    {
      ...
      if(st.tight)
      {
        ...
        std::cerr << "BA succSetSchTightReduced states: " << succ.size() << std::endl;
      }
      else
      {
        succ = succSetSchStartReduced(st.S, rankBound[st.S].bound, reachCons, maxReach, dirRel, oddRel, this->getFinals());
        std::cerr << "BA succSetSchStartReduced states: " << succ.size() << std::endl;
...
map<DFAState, RankBound> BuchiAutomatonSpec::getRankBound(BuchiAutomaton<StateSch, int>& nfaSchewe, set<StateSch>& slignore, map<DFAState, int>& maxReachSize, map<int, int>& minReachSize)
{
  ...
  std::cerr << "rank bound fin size: " << fin.size() << std::endl;
  ...
  std::cerr << "rank bound no fin size: " << nofin.size() << std::endl;
  ...
  if(this->opt.semidetOpt && this->isSemiDeterministic()){
    sd = true;
    std::cerr << "semideterministic" << std::endl;
  }
  ...
  for(const StateSch& s : nfaSchewe.getStates())
  {
    for(vector<int>& sub : Aux::getAllSubsets(vector<int>(s.S.begin(), s.S.end())))
    {
      ...
      std::cerr << "rnkmap[s] = " << rnkmap[s] << std::endl;
      if(sd)
      {
  ...
  auto updPred = [this] (LabelState<StateSch, RankBound>* dest, LabelState<StateSch, RankBound>* pred, map<int, int>* restr)
  {
    ...
    std::cerr << "syms size: " << syms.size() << std::endl;
    //TODO finish
    map<int, int> ret;
    *restr = ret;
  };
  ...
  auto updMaxFnc = [&slignore] (LabelState<StateSch, RankBound>* a, const std::vector<LabelState<StateSch, RankBound>*> sts) -> RankBound
  {
    ...
    std::cerr << ".bound = " << std::min(a->label.bound, m) << std::endl;
  ...
  for(const auto& t : tmp){
    ...
    std::cerr << "ret[t.first.S] = " << t.second.bound << std::endl;
  }  
...
map<DFAState, RankBound> BuchiAutomatonSpec::getRankBound(BuchiAutomaton<StateSchGBA, int>& nfaSchewe, set<StateSchGBA>& slignore, map<DFAState, int>& maxReachSize, map<int, int>& minReachSize)
{
  set<int> nofin;
  set<int> fin = this->getFinals();
  std::cerr << "rank bound fin size: " << fin.size() << std::endl;
  ...
  for(const StateSchGBA& s : nfaSchewe.getStates())
  {
    rnkmap[s] = 0;
  }
  ...
  auto updPred = [this] (LabelState<StateSchGBA, RankBound>* dest, LabelState<StateSchGBA, RankBound>* pred, map<int, int>* restr)
  {
    set<int> syms;
    for(int sym : this->getAlphabet())
    {
  ...
  auto updMaxFnc = [&slignore] (LabelState<StateSchGBA, RankBound>* a, const std::vector<LabelState<StateSchGBA, RankBound>*> sts) -> RankBound
  {
    int m = 0;
    for(const LabelState<StateSchGBA, RankBound>* tmp : sts)
    {
      if(tmp->state.S == a->state.S && slignore.find(a->state) != slignore.end())
        continue;
      m = std::max(m, tmp->label.bound);
  ...
  auto initMaxFnc = [this, &maxReachSize, &minReachSize, &rnkmap] (const StateSchGBA& act) -> RankBound
  {
    set<int> ret;
    set<int> fin = this->getFinals();
    std::set_difference(act.S.begin(),act.S.end(),fin.begin(),
...
map<DFAState, int> BuchiAutomatonSpec::getMaxReachSize(BuchiAutomaton<StateSchGBA, int>& nfaSchewe, set<StateSchGBA>& slIgnore)
{
  auto updMaxFnc = [&slIgnore] (LabelState<StateSchGBA, int>* a, const std::vector<LabelState<StateSchGBA, int>*> sts) -> int
  {
    int m = 0;
    for(const LabelState<StateSchGBA, int>* tmp : sts)
    {
      if(tmp->state.S == a->state.S && slIgnore.find(a->state) != slIgnore.end())
        continue;
      m = std::max(m, tmp->label);
  ...
  auto initMaxFnc = [] (const StateSchGBA& act) -> int
  {
    return act.S.size(); 
  };
...
{
  std::cerr << "get min reach size" << std::endl;
  set<StateSch> slIgnore;
